...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def _eval_predictions(self, predictions):
        ...
        if self._output_dir:
            ...
            zip_file = os.path.join(self._output_dir, f'{timestamp}.zip')
            with zipfile.ZipFile(zip_file, mode="w", compression=zipfile.ZIP_DEFLATED) as archive:
                archive.write(file_path, arcname='results.json')
            self._logger.info("Saving results to {}.".format(zip_file))
    ...
    def _derive_coco_results(self, coco_eval, class_names=None):
        ...
        self._logger.info(
            "Evaluation results for {}: \n".format(
                "segm") + create_small_table(results)
        )
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    for instance_id, (s, l, m) in enumerate(zip(scores, labels, masks)):
        ...
        #     else:
        #         segms.append(mask_util.encode(
        #             np.array(np.zeros((inputs[0]['height'], inputs[0]['width'], 1)), order="F", dtype="uint8"))[0])
        segms = [
            mask_util.encode(
                np.array(_mask[:, :, None], order="F", dtype="uint8"))[0]
            for _mask in m
        ]
        for rle in segms:
