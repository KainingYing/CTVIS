...
YTVIS_CATEGORIES_2022 = YTVIS_CATEGORIES_2021
...
def _get_ytvis_2019_instances_meta():
    ...
    thing_colors = [k["color"]
                    for k in YTVIS_CATEGORIES_2019 if k["isthing"] == 1]
...
def _get_ytvis_2021_instances_meta():
    ...
    thing_colors = [k["color"]
                    for k in YTVIS_CATEGORIES_2021 if k["isthing"] == 1]
    assert len(thing_ids) == 40, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"]
...
def _get_ytvis_2022_instances_meta():
    thing_ids = [k["id"] for k in YTVIS_CATEGORIES_2022 if k["isthing"] == 1]
    thing_colors = [k["color"]
                    for k in YTVIS_CATEGORIES_2022 if k["isthing"] == 1]
    assert len(thing_ids) == 40, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"]
...
"""
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        record = dict()
        record["file_names"] = [os.path.join(
            image_root, vid_dict["file_names"][i]) for i in range(vid_dict["length"])]
        ...
        for frame_idx in range(record["length"]):
            ...
            for anno in anno_dict_list:
                ...
                obj["generate"] = anno.get("generate", False)
...
def register_ytvis_instances(name, metadata, json_file, image_root):
    ...
    DatasetCatalog.register(
        name, lambda: load_ytvis_json(json_file, image_root, name))
